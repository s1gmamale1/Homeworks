...
def test_prompt_version_bumped_to_v6():
    # Bumped v5 -> v6 for the Boss-Arena Why→How→What rework: the generator
    # now also emits scenario + why/how/what reasoning prompts (spec §4/§9).
    text = _PROMPT_PATH.read_text(encoding="utf-8")
    first_line = text.splitlines()[0]
    assert "boss-question-generator:v6" in first_line, (
        f"prompt header must declare v6; got: {first_line!r}"
    )
    assert (
        boss_dynamic.PROMPT_VERSION["boss-question-generator"] == "v6"
    ), "PROMPT_VERSION dict must mirror the prompt-file header"


def test_prompt_emits_why_how_what_chain():
    """Boss-Arena (spec §4/§9): the generator must emit the structured
    scenario + Why→How→What reasoning prompts in addition to the legacy
    fields. Pin those keys so the contract can't silently regress."""
    text = _PROMPT_PATH.read_text(encoding="utf-8")
    for key in ("scenario", "why", "how", "what"):
        assert f'"{key}"' in text, (
            f"prompt must instruct the model to emit a {key!r} field "
            "(Why→How→What chain, spec §4/§9)"
        )
