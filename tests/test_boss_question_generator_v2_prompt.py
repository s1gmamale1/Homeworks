"""Plan Wave 2 — boss-question-generator prompt v2 contract guard.
...
def test_prompt_version_bumped_to_v2():
    ...
    assert "boss-question-generator:v2" in first_line, (
        f"prompt header must declare v2; got: {first_line!r}"
    )
    assert (
        boss_dynamic.PROMPT_VERSION["boss-question-generator"] == "v2"
...
def test_prompt_lists_authored_question_stems_input():
    ...
    assert "authored_question_stems" in text, (
        "prompt v2 must surface authored_question_stems in 'Allowed inputs'"
    )
    assert "authored_difficulty_floor" in text, (
        "prompt v2 must surface authored_difficulty_floor in 'Allowed inputs'"
    )
...
def test_prompt_has_per_skill_floor_rule():
    ...
    assert "Per-skill difficulty floor" in text, (
        "prompt v2 must declare the per-skill difficulty floor as a hard rule"
    )
    ...
    ), "prompt v2 must spell out the one-step-below allowance"
    assert "Anchor to the authored pool" in text, (
        "prompt v2 must declare the anchor-to-authored-pool rule"
    )
