...
def test_prompt_version_bumped_to_v3():
    body = _prompt_text()
    assert "boss-answer-checker:v3" in body, (
        "Prompt version header must say v3 — v2 added language banner "
        "(Bug B); v3 adds semantic-equivalence rule (Bug C)"
    )


def test_prompt_version_map_pinned_to_v3():
    ...
    assert PROMPT_VERSION["boss-answer-checker"] == "v3"
