...
| **Uzbek grammar quality** on new CBP + Memory Check prompts | Per existing rule on native Uzbek linguist review (P7 from FLOW-DEBATE) — defer Uzbek prompt behavior until a signed review exists under `docs/linguist-reviews/`; v2 can ship English/Russian first, with Uzbek marked blocked/draft until signoff. |
